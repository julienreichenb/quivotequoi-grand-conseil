...
OUT_DETAILS_DIR = './output/details'
...
for file in os.listdir(IN_DIR):
    if file.endswith(".xlsx") and file.startswith("Vote"):
        votesFiles.append(os.path.join(IN_DIR,file))

## Mapping file
mapping = pd.read_csv(IN_MAP, delimiter=";")
...
def getAllVotes(file):
	...
	columnstokeep = [mapping.columns[i] for i in [2,3,5,6,7,8,9,11]] #### Remplacer par les colonnes sur le fichier de Romain + traitement des âges (date session - birthyear)
	maps = mapping[columnstokeep].set_index("nom_corresp_vote")
	complete = raw.join(maps, on="name", how="inner")
